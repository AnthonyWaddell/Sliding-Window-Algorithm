...
int clientStopWait(UdpSocket &sock, const int max, int message[])
{
	...
	{
		message[0] = i;
		sock.sendTo((char*)message, BYTE_MESSAGE_SIZE);  // try to send the message
		ack_timer.start(); // get current date information
...
// message in order.
//-----------------------------------------------------------------------------
void serverEarlyRetrans(UdpSocket &sock, const int max, int message[], int dropRate)
{
	cerr << "Testing Server Early Retransmit" << endl;
	vector<bool> array(max, false);
	int ack_buf[1];
	int sequence = 0;
  	int actual_drop_rate = 0;
	bool drop_it;
	...
	while (sequence < max) 
	{
		...
		// Apply the random drop rate for every n between 0 and 10
		actual_drop_rate = (rand() % 100);
		if(actual_drop_rate > dropRate)
		{
			drop_it = true;	
		}
		if(drop_it == true)
